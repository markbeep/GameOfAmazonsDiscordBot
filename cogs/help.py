...
class Help(commands.Cog):
    ...
    async def help(self, ctx):
        embed = discord.Embed(
            ...
            "How to play:\n"
            "Using the `start` command you can start a battle against any othert bot (or user). The player that starts the game is also the one that does the first move. "
            "The players take turns using the `play` command to move one of their valid pieces to a new position and shoot an arrow. "
            ...
            "If by any chance you want to get the current game state, you can also use the `get` command. *For a more human viewable format, use `ama view <match id>`.*\n"
        )
        ...
        embed.add_field(
            ...
            "**Note:** *The locations are to be given in a format as follows: `(x,y)` or `[x,y]` or `{x,y}`*",
            inline=False
        )
